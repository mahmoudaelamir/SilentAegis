...
"""
╔══════════════════════════════════════════════════════════════════════╗
║         POCKET OPTION EXPERT BOT — V17 ULTIMATE PROFESSIONAL        ║
║  ──────────────────────────────────────────────────────────────────  ║
║  INDICATORS (16):                                                    ║
║   RSI • EMA Cross • MACD • Bollinger • Stochastic • ADX             ║
║   Williams %R • CCI • Momentum • ATR Breakout • MTF                 ║
║   Ichimoku Cloud • Fibonacci • Supertrend • Parabolic SAR           ║
║   Heikin Ashi Trend                                                  ║
║                                                                      ║
║  SMART MONEY CONCEPTS:                                               ║
║   Order Blocks • Fair Value Gaps • Liquidity Sweeps                 ║
║   Market Structure (HH/HL/LH/LL) • Supply & Demand Zones           ║
║                                                                      ║
║  CANDLE PATTERNS (14):                                               ║
║   Morning/Evening Star • Engulfing • 3 Soldiers/Crows               ║
║   Hammer • Shooting Star • Pin Bar • Doji • Inside Bar              ║
║   Tweezer • Marubozu • Harami                                        ║
║                                                                      ║
║  FILTERS:                                                            ║
║   Session Quality • ADX Trend Filter • Confluence Gate              ║
║   Hourly Win Rate • Circuit Breaker • Daily Profit Target           ║
║                                                                      ║
║  RISK MANAGEMENT:                                                    ║
║   Dynamic Bet Sizing • Martingale (2 levels) • Drawdown Guard       ║
║   Daily Loss Limit • Daily Profit Lock • Auto Pause                 ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
def _ichimoku(candles):
    """
    Tenkan-sen (9): متوسط أعلى+أدنى / 2 لـ 9 شمعات
    Kijun-sen (26): نفس الفكرة لـ 26 شمعة
    Chikou Span: إغلاق الشمعة الحالية مقارنة بالسعر قبل 26 شمعة
    """
    ...
    # إشارة صاعدة: السعر فوق Kijun، Tenkan فوق Kijun
...
def _fibonacci(candles, lookback=34):
    """يحدد أقرب مستويات فيبوناتشي للسعر الحالي"""
    if len(candles) < lookback: return None
    cs = candles[-lookback:]
    swing_high = max(c["h"] for c in cs)
    swing_low  = min(c["l"] for c in cs)
    ...
    for name, lvl in levels.items():
        if abs(price - lvl) <= margin:
            # اتجاه الارتداد بناءً على موضع السعر
            if price < swing_high - rng*0.4:  # في النصف السفلي → ارتداد صاعد
                return "CALL", int(70 + float(name)*0.1), f"Fib {name}% Support"
            else:
                return "PUT", int(70 + float(name)*0.1), f"Fib {name}% Resistance"
    return None
...
def _supertrend(candles, period=10, multiplier=3.0):
    ...
    price = candles[-1]["c"]
    # بسيط: هل السعر فوق أو تحت Supertrend
    prev_hl2 = (candles[-2]["h"] + candles[-2]["l"]) / 2
    prev_price= candles[-2]["c"]
    if price > lower and prev_price <= (prev_hl2 - multiplier*atr):
        return "CALL", 78, f"Supertrend↑"
    if price < upper and prev_price >= (prev_hl2 + multiplier*atr):
        return "PUT", 78, f"Supertrend↓"
...
def _parabolic_sar(candles, af_start=0.02, af_max=0.20):
    """
    Parabolic SAR — نقطة وقف واستدارة
    SAR تحت السعر = صاعد، فوق السعر = هابط
    """
    if len(candles) < 10: return None
    cs = candles[-20:] if len(candles)>=20 else candles
    bull = cs[1]["c"] > cs[0]["c"]
    sar  = cs[0]["l"] if bull else cs[0]["h"]
    ep   = cs[0]["h"] if bull else cs[0]["l"]
    af   = af_start
    for i in range(1, len(cs)):
        sar = sar + af * (ep - sar)
        if bull:
            if cs[i]["l"] < sar:
                bull=False; sar=ep; ep=cs[i]["l"]; af=af_start
            else:
                if cs[i]["h"] > ep: ep=cs[i]["h"]; af=min(af+af_start,af_max)
                sar = min(sar, cs[i-1]["l"], cs[i]["l"] if i>1 else sar)
        else:
            if cs[i]["h"] > sar:
                bull=True; sar=ep; ep=cs[i]["h"]; af=af_start
            else:
                if cs[i]["l"] < ep: ep=cs[i]["l"]; af=min(af+af_start,af_max)
                sar = max(sar, cs[i-1]["h"], cs[i]["h"] if i>1 else sar)
    price = cs[-1]["c"]
    dist  = abs(price-sar)/price*100
    if bull:
        return "CALL", min(85, 68+int(dist*100)), f"PSAR Bull({dist:.3f}%)"
    else:
        return "PUT",  min(85, 68+int(dist*100)), f"PSAR Bear({dist:.3f}%)"

# ── Heikin Ashi Trend ──────────────────────────────────────
def _heikin_ashi_trend(candles, lookback=5):
    """
    Heikin Ashi يُصفّي الضوضاء ويظهر الاتجاه الحقيقي
    4 شمعات HA حمراء = اتجاه هابط قوي
    4 شمعات HA خضراء = اتجاه صاعد قوي
    """
    if len(candles) < lookback+1: return None
    ha_candles = []
    for i in range(len(candles)):
        ha_c = (candles[i]["o"]+candles[i]["h"]+candles[i]["l"]+candles[i]["c"])/4
        if i==0:
            ha_o = (candles[i]["o"]+candles[i]["c"])/2
        else:
            ha_o = (ha_candles[-1]["o"]+ha_candles[-1]["c"])/2
        ha_h = max(candles[i]["h"], ha_o, ha_c)
        ha_l = min(candles[i]["l"], ha_o, ha_c)
        ha_candles.append({"o":ha_o,"h":ha_h,"l":ha_l,"c":ha_c})
    recent = ha_candles[-lookback:]
    bull_count = sum(1 for c in recent if c["c"] > c["o"])
    bear_count = sum(1 for c in recent if c["c"] < c["o"])
    # تحقق من قوة الجسم (بدون ذيول = إشارة أقوى)
    last = ha_candles[-1]
    no_lower = last["l"] == min(last["o"],last["c"])
    no_upper = last["h"] == max(last["o"],last["c"])
    if bull_count >= 4:
        conf = 75 + (5 if no_lower else 0)
        return "CALL", conf, f"HA Bull x{bull_count}"
    if bear_count >= 4:
        conf = 75 + (5 if no_upper else 0)
        return "PUT", conf, f"HA Bear x{bear_count}"
    return None
...
def _market_structure(candles, lookback=20):
    """
    يكتشف هيكل السوق:
    HH + HL = اتجاه صاعد قوي
    LH + LL = اتجاه هابط قوي
    Break of Structure (BOS) = إشارة دخول قوية
    """
    if len(candles) < lookback: return None
    cs = candles[-lookback:]
    ...
    if len(peaks)>=2 and len(troughs)>=2:
        hh = cs[peaks[-1]]["h"]  > cs[peaks[-2]]["h"]   # Higher High
        lh = cs[peaks[-1]]["h"]  < cs[peaks[-2]]["h"]   # Lower High
        hl = cs[troughs[-1]]["l"]> cs[troughs[-2]]["l"] # Higher Low
        ll = cs[troughs[-1]]["l"]< cs[troughs[-2]]["l"] # Lower Low
        # BOS صاعد: سعر يكسر أعلى قمة سابقة
        if hh and hl:
            return "CALL", 82, "Market Structure HH+HL"
        if lh and ll:
            return "PUT",  82, "Market Structure LH+LL"
        if hh:
            return "CALL", 72, "Higher High (BOS)"
        if ll:
            return "PUT",  72, "Lower Low (BOS)"
    return None

def _order_blocks(candles, lookback=15):
    """
    Order Block = آخر شمعة عكسية قبل حركة قوية
    (المؤسسات تضع أوامرها هنا)
    """
    if len(candles) < lookback: return None
    cs = candles[-lookback:]
    price = cs[-1]["c"]
    for i in range(len(cs)-3, max(0,len(cs)-10), -1):
        c0,c1,c2 = cs[i], cs[i+1], cs[i+2]
        body0 = abs(c0["c"]-c0["o"])
        # Bullish OB: شمعة هابطة تتبعها 2 شمعات صاعدة قوية
        if (c0["c"]<c0["o"] and c1["c"]>c1["o"] and c2["c"]>c2["o"]
                and (c1["c"]-c1["o"])>body0*0.5):
            ob_top = c0["h"]; ob_bot = c0["l"]
            if ob_bot <= price <= ob_top:
                return "CALL", 84, f"Bullish OB Zone"
        # Bearish OB: شمعة صاعدة تتبعها 2 شمعات هابطة قوية
        if (c0["c"]>c0["o"] and c1["c"]<c1["o"] and c2["c"]<c2["o"]
                and (c1["o"]-c1["c"])>body0*0.5):
            ob_top = c0["h"]; ob_bot = c0["l"]
            if ob_bot <= price <= ob_top:
                return "PUT", 84, f"Bearish OB Zone"
    return None

def _fair_value_gap(candles):
    """
    FVG = فجوة بين شمعتين تتخطاهما الشمعة الوسطى
    السعر يعود ليملأ الفجوة = فرصة دخول
    """
    if len(candles) < 4: return None
    c1,c2,c3 = candles[-3], candles[-2], candles[-1]
    price = c3["c"]
    # Bullish FVG: c1.high < c3.low (فجوة صاعدة)
    if c1["h"] < c3["l"]:
        gap_mid = (c1["h"] + c3["l"]) / 2
        if abs(price - gap_mid) / price < 0.001:
            return "CALL", 77, f"Bullish FVG Fill"
    # Bearish FVG: c1.low > c3.high (فجوة هابطة)
    if c1["l"] > c3["h"]:
        gap_mid = (c1["l"] + c3["h"]) / 2
        if abs(price - gap_mid) / price < 0.001:
            return "PUT", 77, f"Bearish FVG Fill"
    return None

def _liquidity_sweep(candles, lookback=20):
    """
    Liquidity Sweep = اختراق قمة/قاع سابقة ثم ارتداد سريع
    (الـ Smart Money تصيد الـ Stop Loss)
    """
    if len(candles) < lookback: return None
    cs = candles[-lookback:]
    recent_high = max(c["h"] for c in cs[:-3])
    recent_low  = min(c["l"] for c in cs[:-3])
    last = cs[-1]; prev = cs[-2]
    # Sweep صاعد: اخترقت القاع ثم أغلقت فوقه
    if prev["l"] < recent_low and last["c"] > recent_low:
        return "CALL", 86, f"Liquidity Sweep Low"
    # Sweep هابط: اخترقت القمة ثم أغلقت تحتها
    if prev["h"] > recent_high and last["c"] < recent_high:
        return "PUT", 86, f"Liquidity Sweep High"
    return None

def _supply_demand_zones(candles, lookback=30):
    """
    مناطق العرض والطلب الاحترافية:
    - Rally-Base-Drop (RBD) = منطقة عرض
    - Drop-Base-Rally (DBR) = منطقة طلب
    """
    if len(candles) < lookback: return None
    cs = candles[-lookback:]
    price = cs[-1]["c"]
    for i in range(2, len(cs)-2):
        base = cs[i]
        base_size = abs(base["c"]-base["o"])
        before_bull = cs[i-1]["c"] > cs[i-1]["o"]
        before_move = abs(cs[i-1]["c"]-cs[i-1]["o"])
        after_drop  = cs[i+1]["c"] < cs[i+1]["o"]
        after_move  = abs(cs[i+1]["c"]-cs[i+1]["o"])
        # RBD: صاعد → قاعدة → هابط (منطقة عرض)
        if before_bull and after_drop and after_move > before_move*1.5:
            z_top = max(base["h"], cs[i-1]["h"])
            z_bot = min(base["l"], cs[i-1]["l"])
            if z_bot <= price <= z_top:
                return "PUT", 80, f"Supply Zone (RBD)"
        # DBR: هابط → قاعدة → صاعد (منطقة طلب)
        before_bear = cs[i-1]["c"] < cs[i-1]["o"]
        after_rally = cs[i+1]["c"] > cs[i+1]["o"]
        if before_bear and after_rally and after_move > before_move*1.5:
            z_top = max(base["h"], cs[i+1]["h"])
            z_bot = min(base["l"], cs[i+1]["l"])
            if z_bot <= price <= z_top:
                return "CALL", 80, f"Demand Zone (DBR)"
    return None

# ══════════════════════════════════════════════════════════
#  نماذج الشمعات — 14 نموذج
# ══════════════════════════════════════════════════════════
...
def _is_marubozu(c, threshold=0.95):
    """Marubozu = جسم كبير بدون ذيول تقريباً"""
    b=abs(c["c"]-c["o"]); r=c["h"]-c["l"]
    return r>0 and b/r>=threshold

def _is_harami(c1,c2):
    """Harami = شمعة صغيرة داخل جسم الشمعة السابقة الكبيرة"""
    big=abs(c1["c"]-c1["o"]); sm=abs(c2["c"]-c2["o"])
    c2_inside = min(c1["o"],c1["c"])<=c2["o"]<=max(c1["o"],c1["c"]) and \
                min(c1["o"],c1["c"])<=c2["c"]<=max(c1["o"],c1["c"])
    return big>0 and sm<big*0.5 and c2_inside
...
# ══════════════════════════════════════════════════════════
#  محرك الاستراتيجيات — 20+ مصدر إشارة
# ══════════════════════════════════════════════════════════
class StrategyEngine:

    # أوزان كل مجموعة
    W = {
        "smc":    2.5,   # Smart Money Concepts — الأعلى وزناً
        "candle": 2.2,   # نماذج الشمعات
        "sr":     2.0,   # دعم ومقاومة وPivot
        "mtf":    2.1,   # Multi-Timeframe
        "ichi":   1.9,   # Ichimoku
        "ema":    1.9,   # EMA Cross
        "fib":    1.8,   # Fibonacci
        "bb":     1.8,   # Bollinger
        ...
        "atr":    1.5,   # ATR
        "cci":    1.5,   # CCI
        "psar":   1.4,   # Parabolic SAR
        "super":  1.5,   # Supertrend
        "will":   1.4,   # Williams
    }

    def run_all(self, candles):
        if len(candles) < 8:
            return "CALL",52,"Warming Up","System",0
        ...
        adx_ok  = adx_val is None or adx_val >= 18
        ...
        # ─── Smart Money Concepts ───────────────────────────
        add(_liquidity_sweep(candles),    "smc")
        add(_order_blocks(candles),       "smc")
        add(_fair_value_gap(candles),     "smc")
        add(_market_structure(candles),   "smc")
        ...
        total=call_w+put_w
        if total==0:
            return "CALL",52,"No Signal","Market",0

        # عدد الإشارات المتفقة (Confluence)
        if call_w>=put_w:
            con=int((call_w/total)*100)
            fin=min(95,int(con*0.5+bc_call*0.5))
            # علاوة الإجماع
            ...
            con=int((put_w/total)*100)
            fin=min(95,int(con*0.5+bc_put*0.5))
    ...
    def _candle_patterns(self,cs,c1,c2,c3,c4):
        ...
        if _is_inside_bar(c3,c4):
            trend_up=cs[-5]["c"]<cs[-3]["c"] if len(cs)>=5 else True
            return ("CALL",72,"Inside Bar↑") if trend_up else ("PUT",72,"Inside Bar↓")
        if _is_doji(c3):
    ...
    def _rsi_strat(self,closes):
        ...
        ro=_rsi(closes[:-3],14)
        if ro:
            if rsi<45 and closes[-1]<closes[-3] and rsi>ro: return "CALL",78,"RSI Bull Div"
            if rsi>55 and closes[-1]>closes[-3] and rsi<ro: return "PUT", 78,"RSI Bear Div"
        return None
    ...
    def _bb_strat(self,closes):
        ...
        bwp_up,_,bwp_lo=_bollinger(closes[:-1],20)
        bwp=((bwp_up or 0)-(bwp_lo or 0))/mid
        sq=bw<bwp*0.8
        if p<=lo:
    ...
    def _stoch_strat(self,highs,lows,closes):
        ...
        kp=_stochastic(highs[:-1],lows[:-1],closes[:-1],14)
        ...
        if kp and kp<50<=k: return "CALL",68,"Stoch Cross50↑"
        if kp and kp>50>=k: return "PUT", 68,"Stoch Cross50↓"
        return None
    ...
    def _sr_pivot(self,closes,highs,lows,candles):
        ...
        if rng==0: return None
        m=rng*0.035
        if pp and abs(p-s1)<m*1.5: return "CALL",85,f"Pivot S1"
        if pp and abs(p-r1)<m*1.5: return "PUT", 85,f"Pivot R1"
    ...
    def _williams_strat(self,highs,lows,closes):
        ...
        wrp=_williams_r(highs[:-1],lows[:-1],closes[:-1],14)
    ...
    def _mtf_trend(self,closes):
        ...
        if e8>e21>e50:
            gap=(e8-e50)/e50*100
            return "CALL",min(88,68+int(gap*500)),f"MTF Bull {gap:.3f}%"
        if e8<e21<e50:
            gap=(e50-e8)/e50*100
            return "PUT",min(88,68+int(gap*500)),f"MTF Bear {gap:.3f}%"
        return None
...
class RiskManager:
    def __init__(self):
        ...
        self.max_daily_loss  = 0.20   # 20% من الرصيد
        self.daily_profit_tgt= 0.10   # 10% هدف يومي
        self.consecutive_L   = 0
        self.martingale_lvl  = 0
        self.circuit_break   = False
        self.cb_until        = None
        self.profit_locked   = False
        self.total_trades    = 0
        self.total_wins      = 0
        self.hourly_wins     = {}  # {hour: [W,L]}
    ...
    def get_bet(self, conf, smult, confluence):
        ...
        if self.profit_locked:
            return 0, "✅ Daily Target Reached — Locked"
    ...
    def best_hour(self):
        ...
        for h,(w,l) in self.hourly_wins.items():
            if w+l>=3:
                wr=w/(w+l)*100
                if wr>best_wr: best_wr=wr; best=h
        return f"Best Hour: {best:02d}:00 ({best_wr:.0f}%)" if best else ""

    def status_line(self):
        icon="🔴" if self.circuit_break else ("🟡" if self.martingale_lvl>0 else ("✅" if self.profit_locked else "🟢"))
        return f"{icon} ${self.balance:.0f} | WR:{self.winrate():.0f}% | DD:{self.drawdown():.1f}%"
...
class TargetCrosshair:
    ...
    def _drag(self,e):
        x=self.win.winfo_x()+(e.x-self._xy[0])
        y=self.win.winfo_y()+(e.y-self._xy[1])
...
class App:
    def __init__(self):
        ...
        self.SW=SW; self.SH=SH; self.H=110
        ...
        self.v_min_conf_auto=tk.IntVar(value=80)  # حد أعلى للدخول الآلي
    ...
    def _build(self):
        bar=tk.Frame(self.root,bg="#050810"); bar.pack(fill="both",expand=True)
        ...
        # عمود 1 — معلومات النظام
        c1=self._col(bar,112)
        tk.Label(c1,text="ULTIMATE V17",bg="#050810",fg="#00ff44",
        ...
        self.lbl_besthour=tk.Label(c1,text="",bg="#050810",fg="#445544",font=("Consolas",6))
        self.lbl_besthour.pack(anchor="w",padx=3)

        # عمود 2 — إعدادات
        c2=self._col(bar,225)
        ttk.Combobox(c2,textvariable=self.v_pair,values=list(PAIRS.keys()),
                     width=16,font=("Consolas",8)).pack(anchor="w",padx=3,pady=(4,1))
        ...
        tk.Label(r2,text="Signal%:",bg="#050810",fg="#555",font=("Consolas",7)).pack(side="left")
        tk.Spinbox(r2,from_=60,to=95,textvariable=self.v_min_conf,
        ...
        tk.Spinbox(r2,from_=70,to=95,textvariable=self.v_min_conf_auto,
                   width=3,font=("Consolas",7),bg="#111",fg="#aaa").pack(side="left")
        ...
        tk.Label(c2,text="● 20+ Signals | SMC | Ichimoku | Fibonacci",
                 bg="#050810",fg="#A78BFA",font=("Consolas",7,"bold")).pack(anchor="w",padx=3)
        ...
        br=tk.Frame(c7,bg="#050810"); br.pack(anchor="w",padx=3,pady=(35,0))
        for t,bg,fg,cmd in [
    ...
    def _price_loop(self):
        def _run():
            ...
            if p is not None:
                if is_otc: p+=random.uniform(-0.00010,0.00010)
                self.last_valid_price=p
                self.root.after(0,lambda:self._on_price(p))
            self.root.after(750,self._price_loop)
    ...
    def _analyze(self):
        ...
        self.lbl_wr.config(text=f"AI Trust:{wr:.0f}%",
        ...
        self.lbl_confl.config(text=f"Confluence: {cf} signals",fg=confl_clr)
        ...
        ok_session = sq>=55
        ok_circuit = not self.risk.circuit_break
        ok_profit  = not self.risk.profit_locked
        ok_conf    = c>=auto_min
        ok_confl   = cf>=3  # لا تدخل بأقل من 3 إشارات متفقة

        if self.auto_click and self.targets_ready and ok_conf and d and ok_session and ok_circuit and ok_profit and ok_confl:
    ...
    def _draw(self,changed=False):
        ...
        if not d:
            ...
            self.lbl_conf.config(text="Analyzing 20+ Signals...",fg="#444")
        ...
        cf_stars="⬥"*min(self.confluence,6)
        self.lbl_conf.config(text=f"{c}%  {stars}  {cf_stars}  [{self.strat_name}]",fg=clr)
